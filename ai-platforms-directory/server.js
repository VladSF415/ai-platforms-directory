...
import { trackPageView, getServerAnalytics } from './server-analytics.js';
...
// ===========================================
// SERVER-SIDE ANALYTICS: Track all page views (cannot be blocked by ad blockers)
// ===========================================
fastify.addHook('onRequest', async (request, reply) => {
...
// Server-side analytics endpoint (TRUE traffic numbers - cannot be blocked by ad blockers)
fastify.get('/api/server-analytics', async (request, reply) => {
  try {
    const analytics = getServerAnalytics();
    if (!analytics) {
      reply.code(500).send({ error: 'Failed to load server analytics' });
      return;
    }
    return analytics;
  } catch (error) {
    console.error('[Server Analytics] Error:', error);
    reply.code(500).send({ error: 'Failed to retrieve server analytics' });
  }
});
