  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (contentResponse.ok) {
        ...
        // If pillar content has specific platforms, fetch those
        if (content.platforms && content.platforms.length > 0) {
          // Fetch all platforms and filter by IDs
          const platformsResponse = await fetch('/api/platforms?limit=2000');
          const platformsData = await platformsResponse.json();
          const allPlatforms = platformsData.platforms || [];

          // Filter to only platforms in the pillar content
          const linkedPlatforms = allPlatforms.filter((p: Platform) =>
            content.platforms.includes(p.id)
          );

          // Sort to match the order in pillar content (featured first)
          const sorted = linkedPlatforms.sort((a: Platform, b: Platform) => {
            const aIndex = content.platforms.indexOf(a.id);
            const bIndex = content.platforms.indexOf(b.id);

            // Featured platforms come first
            const aFeatured = content.featured?.includes(a.id);
            const bFeatured = content.featured?.includes(b.id);
          ...
          // Fallback: Fetch platforms by category
